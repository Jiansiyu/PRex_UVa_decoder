...
//map<int, TH1F* > RawDecoder::DrawRawHisto(TCanvas *c0,TCanvas *c1,TCanvas *c2,TCanvas *c3, TCanvas *c4,TCanvas *c5,TCanvas *c6,TCanvas *c7, TCanvas *c8, TCanvas *c9, TCanvas *c10, TCanvas *c11)
//map<int, TH1F* > RawDecoder::DrawRawHisto(TCanvas *c0,TCanvas *c1,TCanvas *c2)
{
  int mpd_id=0;
  int adc_ch=0;
  int hybridID=0;
  ...
  for(it = mAPVRawSingleEvent.begin(); it!=mAPVRawSingleEvent.end(); ++it)
    {
      ...
      //      map<TString, int> histnameToAPVIDmap ;
      ...
      //      histnameToAPVIDmap[h->GetName()] = (mpd_id << 4) | adc_ch ;
  ...
  for(itRaw = mAPVRawHisto.begin();itRaw!=mAPVRawHisto.end();itRaw++)
    {
     ...
     j0=i;
      ...
      j10=i-160;
      j11=i-176;
      //      cout<<"i: "<<i<<"=> j0: "<<j0<<" j1: "<<j1<<" j2: "<<j2<<" j3: "<<j3<<" j4: "<<j4<<" j5: "<<j5<<endl;
      */

      //      TString histoname = itRaw->second->GetName() ;
      //      TString mpdname =  histoname.Remove(0,4);
      //      cout<<"histoname: "<<histoname<<" mpdname: "<<mpdname<<endl;

      c->cd(i);
      itRaw->second->SetTitleSize(0.2);
      itRaw->second->SetMaximum(3000);
      itRaw->second->SetMinimum(400);
      itRaw->second->Draw();
      /**
      if(i<17) {
	cout<<"i: "<<i<<"=> j0: "<<j0<<endl;
	c0->cd(j0);
	itRaw->second->SetTitleSize(0.5);
	itRaw->second->SetMaximum(2200);
	itRaw->second->SetMinimum(500);
	itRaw->second->Draw();
      }
      if((i>16) && (i<33)) {
	cout<<"i: "<<i<<"=> j1: "<<j1<<endl;
  ...
  if ((typc==83) || (typc==115)) { // if s or S has pressed, it save the histos in a png file
    ...
    //    c0->SaveAs("Result/rawhisto_MPD1.png");
    //    c1->SaveAs("Result/rawhisto_MPD0.png");
    //   c2->SaveAs("Result/rawhisto_MPD2.png");
  ...
  if ((typc==80) || (typc==112)) { // if p or P has pressed, it save the histos in a pdf file
    ...
    //    c0->SaveAs("Result/rawhisto_MPD1.pdf");
    //    c1->SaveAs("Result/rawhisto_MPD0.pdf");
    //    c2->SaveAs("Result/rawhisto_MPD2.pdf");
    getchar(); // return key
  }
